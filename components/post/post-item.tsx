...
const PostItem: React.FC<PostItemProps> = ({
  ...
  return (
    <div className="w-full shadow-md  bg-zinc-200 rounded-md  dark:bg-gradient-to-b dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 animate-fade animate-duration-[1000ms] transition-all animate-ease-in">
      <div className="flex flex-col sm:flex-row justify-between px-5 py-5 gap-3">
        <div className="flex flex-col-reverse sm:flex-row items-center justify-center gap-1 w-full">
          {profile && (
            <ProfileAvatar profile={profile} currentProfile={currentProfile} />
          )}
        ...
        <div className="flex items-center justify-center sm:flex-col border-t-1 border-l-1 border-r-1 sm:border-r-0 sm:border-b-1 rounded-md  border-black/30">
          {currentProfile.id === profile.id && (
            <div className="flex items-center justify-center py-3 px-2">
              <Tooltip placement="bottom" content="Edit Post">
                <Button
                  onClick={handleOpen}
                  size="sm"
                  className="shadow-md  p-0 dark:bg-zinc-400 flex items-center justify-center rounded-md"
                >
                  <GripHorizontal />
                </Button>
              </Tooltip>
          ...
          <div className="min-h-[4rem]  flex flex-col gap-1 items-center justify-center px-3 py-1 ">
            ...
            <p className="text-muted-foreground ">
              {createdAt.getMonth()}/{createdAt.getDay()}/
