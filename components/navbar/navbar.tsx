...
<nav className="fixed w-full h-[50px] flex items-center justify-between px-2 z-50 bg-gradient-to-b from-zinc-700 to-slate-800 shadow-xl ">
    ...
    {!user ? (
      ...
      <p className={cn("font-bold text-md md:text-3xl font-sans bg-gradient-to-r from-sky-400 to-slate-300 bg-clip-text text-transparent",headingFont.className)}>Group-Up</p>
    </Link>
    ):(
  <Link className="flex items-center justify-center gap-1" href="/dashboard">  
      <Image src="/mlogo.png" width={40} height={40} alt="logo" />
      <p className={cn("font-bold text-md md:text-3xl font-sans bg-gradient-to-r from-sky-500  to-slate-300 bg-clip-text text-transparent",headingFont.className)}>Group-Up</p>
    </Link>
    )}
